...
function protocolItems1214(resolved, hierDir, model, route, sessionId) {
  ...
  return [
    ...
    `13. PEER ROSTER + ROUTE — ${dirText}/peers.jsonl is ground truth for which role peers are up, seen, or briefed; after compaction trust the HIERARCHY STATE block over memory. This session's dispatch route is ${routeText} — honor it without re-asking (a PreToolUse gate enforces it, one-shot per role per session, on the roster roles). If the user changes it in chat ("peers only", "subagents only", "prefer peers"/"peers when free"), record it immediately with \`${routeCmd}\` and confirm in one line. Standing up, reshaping, or tearing down a live Team — create/spawn a Team, spawn or dismiss one member, add/edit/remove a roster role, disband, resync/move — goes through the \`ah:agent-roster\` skill (\`Skill(skill:"ah:agent-roster")\`, or \`/agent-roster\`), not a raw roster MCP call; a one-off subagent dispatch of a role is ordinary protocol and needs no skill.`,
