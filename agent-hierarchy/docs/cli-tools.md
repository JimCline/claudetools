...
| add roster member | `node <R>/hooks/roster.mjs add [level] [--level <L>] --role <R> [--model <M>] [--effort <E>] [--route peer\|subagent\|pane] [--kind <K>] [--args '<json>'] [--auto-mode <A>] [--on-missing auto\|prompt\|never]` — `--on-missing` defaults to `auto` |
